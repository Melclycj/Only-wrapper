...
---
*Last updated: 2026-06-09 — Phase 6.1 complete (TERM-09 + TERM-12, terminal-lifecycle state-machine + agent-state redesign; supersedes the failed Phase-6 idle-detection sign-off). Delivered the two-bucket lifecycle (Working Area + Inactive List, no Stop verb; header Clear+Remove; Start on Inactive entries), frame-stability agent-state detection (the persistent `❯` input-box arrow excluded so `claude --rc` is amber on a real confirmation prompt, never stuck blue), and the Core-Value scroll/`[%30/]` alt-screen fidelity fix. Interface-first across 4 plans / 3 waves; the end-of-phase human-verify failed twice and was driven to PASS over 4 hands-on rounds (amber settle-independence, readiness-timer revert, Remove→Inactive `'stopped'` routing, flush-on-close persistence, single Start button). Then a code-review gate found + fixed 4 criticals (dock-relaunch quit-flag, concurrent-restart lock, non-atomic store flush, setOrder clamp). 267 unit + 14/14 smoke GREEN; tsc + eslint clean; nyquist_compliant flipped true for both Phase 6 and 6.1; crash_frame_policy = preserve (Open-Q1). Next: Phase 7 — terminal search + scrollback config.*
