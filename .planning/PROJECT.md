...
- [x] Real interactive PTY terminal surface — a true PTY (not one-shot capture): keyboard, Ctrl+C/Ctrl+D, arrow keys, copy/paste, resize, ANSI colours, long-running + interactive programs (vim / ssh / REPLs) — *Phase 2 (TERM-01/02; Core-Value fidelity human-confirmed at the Phase-3 checkpoint)*
- [x] Normal shell mode plus a configured initial working directory; manual `cd` and launch tools from any accessible folder — *Phase 2 (TERM-03/04)*
- [x] Platform-aware shell selection for **Windows** (PowerShell/CMD/Git Bash/WSL) — *Phase 8 (D-02): `WindowsShellProvider` enumeration behind the Phase-5 seam (env-expanded paths, never-empty default); real-Windows install-path correctness deferred to Windows-hardware UAT (08-HUMAN-UAT.md)*
- [x] Package as a local desktop app for both Windows and macOS from one codebase — *Phase 8 (PKG-01): `npm run make` → runnable macOS `.app` (icon + `com.justwrapper.app`, ASAR-unpacked node-pty, native ConPTY pre-1809 gate, env-gated/unsigned sign slots) + a GitHub Actions 2-OS matrix producing the Windows `.exe`/installer. macOS canonical `claude --rc` scenario user-verified 2026-06-10; Windows real-hardware run deferred (08-HUMAN-UAT.md)*
...
- [ ] **Whole-app visual polish** — one cohesive look & feel pass across sidebar, terminal chrome, create/edit form, spacing rhythm, empty/loading/error states, and hover/focus/active states; a design direction + tokens are locked at plan time, then applied across surfaces (not piecemeal)
  - Foundation validated in Phase 9 (UI-01): `tokens.css` single-source `:root` token layer (28× blue / 8× danger / fonts / shadows / motion migrated value-preservingly, completeness-guard test), self-hosted Nunito + JetBrains Mono rendering and packaging-proven (`verify:fonts`), SC3 one-token re-theme human-verified 2026-06-11. Per-surface application = Phases 10–13 (5 pixel-level gaps logged in 09-HUMAN-UAT.md; ui-lab screenshot harness now available for the look→edit→re-look loop). **Phase 10 (UI-02) complete 2026-06-13** — sidebar visual polish approved live across 5 human gates: two-line row hierarchy, unmistakable active row, legible 5-state status, scroll-invariant agent status (GAP-10-J), and CJK terminal rendering (GAP-10-K); GAP-10-A..K all closed. Phases 11–13 remain.
...
|----------|-----------|---------|
| Cross-platform (Windows + macOS) | Dev/test happens on macOS but the user's primary machine is Windows; fidelity (Core Value) must be testable where it's built. `node-pty` already abstracts ConPTY vs forkpty, so the cost is modest and confined to the edges. | ✓ Good — single codebase ships both; edges (shell list, paths, packaging) stayed isolated. Windows real-hardware verification is the one deferred tail. |
| Core Value = real terminal fidelity | Hosting `claude --rc`, `codex`, vim, ssh, REPLs correctly is the whole point; a fake/captured terminal would be useless. | ✓ Good — canonical `claude --rc` + vim/REPL fidelity human-verified on macOS; the alt-screen `[%30/]` regression was caught and fixed in Phase 6.1. |
| Real PTY layer (not run→capture→return) | Interactive programs require a live pseudo-terminal; one-shot execution can't support them. | ✓ Good — node-pty PTY per session; no capture path ever shipped. |
| Decouple logical session ID from process ID | Identity must survive restarts, renames, and tab switches; process IDs are inherently temporary. | ✓ Good — branded `LogicalId` distinct from `ptyPid`; restart reuses id + new pid, integration-verified across PtyManager + SessionStore. |
| Desktop framework chosen via research | User has no strong preference (Electron vs Tauri); let research recommend based on PTY integration + packaging weight. | ✓ Good — Electron + Electron Forge + node-pty; ASAR-unpack + N-API prebuild packaging proven on macOS, CI-built on Windows. |
| Browser companion deferred / kept separate | Explicitly out of MVP to protect scope and privacy; lives as a future optional feature. | ✓ Good — stayed fully out of v1.0; BROW-01/02/03 remain v2 candidates. |
...
---
*Last updated: 2026-06-13 — **Phase 9 complete — Design Token Foundation (UI-01)**: `src/renderer/tokens.css` is the single source of truth (surface/ink/line + terminal palette + brand/danger + 5 status accents + radius/shadow/motion/font + defined-not-applied `--space-*` scale); self-hosted `@fontsource` Nunito + JetBrains Mono exact-pinned, imported fonts→tokens→terminal.css, and proven to survive packaging (`verify:fonts`: 23 woff2, no absolute urls); terminal.css + status-colors.ts migrated to `var()`/`color-mix` value-preservingly with a pure-node completeness-guard test (verifier 8/8 must-haves; 326 unit GREEN; human-verify approved with SC3 one-token re-theme proven via screenshots). Session extras: **ui-lab visual harness** added (user-directed, commit 81192cd — wdio-based 9-surface screenshot capture + DESIGN-RUBRIC + self-evolve loop + isolated-userData seam) — Phases 10–13 use it for the look→edit→re-look polish loop; 5 pixel-level visual gaps logged (sidebar row-name crush, unframed terminal, weak Save button, context-menu position/danger-color, rail hierarchy) in 09-HUMAN-UAT.md; 4 flaky smoke specs recorded for quiet-machine re-baseline before v1.1 ship (todo 2026-06-11). **Phase 10 complete 2026-06-13 — Sidebar Visual Polish (UI-02)**: two-line row hierarchy, unmistakable active row, legible 5-state status, scroll-invariant agent status (GAP-10-J fix — classifier reads the live tail `baseY`, not the scroll-moved `viewportY`), and CJK terminal rendering (GAP-10-K — UTF-8 spawn locale + CJK font fallback). Approved live across 5 human gates; GAP-10-A..K all closed; round-5 delta review clean (0 Crit/High); unit 384 + smoke 15/15 green against the packaged app. Next: Phase 11 — Terminal Area Polish + Live Start/Restart (UI-03, SESS-07).*
