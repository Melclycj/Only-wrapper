...
- [x] **Phase 6: Robustness + Flow-Control Polish** - ⚠️ SUPERSEDED by Phase 6.1 — the original output-silence idle-detection model failed human-verify and was redesigned. TERM-09/TERM-12 were ultimately delivered in Phase 6.1 (superseded 2026-06-09)
- [x] **Phase 6.1: Terminal Lifecycle Redesign** (INSERTED) - Two-bucket Working Area + Inactive List (no "Stop" verb), frame-stability "waiting for you" amber detection, and the alt-screen `[%30/]` fidelity fix; replaces the failed Phase 6 idle model (TERM-09/TERM-12; completed 2026-06-09)
