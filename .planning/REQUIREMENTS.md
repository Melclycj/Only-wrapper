...
- [ ] **TERM-05**: A session can optionally run a configured startup command after opening, separate from normal shell mode (FR-16) — **Phase 5.1** (un-deferred; descoped from Phase 3, re-homed to its own phase after Phase 5 persistence — runs the saved command into the PTY once the shell is ready, on start/restart)
...
| TERM-05 | Phase 5.1: TERM-05 startup-command auto-run (INSERTED) | Pending |
