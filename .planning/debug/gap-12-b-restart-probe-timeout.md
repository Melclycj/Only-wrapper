...
- timestamp: 2026-06-16 (Round 3 — IN-APP diagnostic instrument BUILT + packaged)
  checked: src/main/pty-manager.ts create() probe block (~L466-665). Added 5 marked `// DIAG GAP-12-B — REMOVE when the real fix lands` blocks; built with `npm run package`; type-checked with `npx tsc --noEmit`; verified the DIAG is bundled inside the packaged app.asar.
  found:
    - INSTRUMENT (purely additive, in-MEMORY, gated `process.env.JW_PROBE_DIAG !== '0'` → ON by default, disableable): (1) a `diag` state object + `diagWrite()` appender declared after the `timers` holder; (2) `diag.t0 = Date.now()` captured at the `child.write(probe.marker)` call (the true probe→ready window origin); (3) in the onData handler, after `buffer += data`, push `{ dtMs: now - t0, size: data.length }` (NO byte contents) + track firstByteMs / maxSilentGapMs / byteCount / chunkCount; (4) in the match branch, capture `diagMatchMs` then `diagWrite('match', diagMatchMs)` AFTER the inject (post-settle, so file I/O can't perturb the window); (5) in giveUpReadiness, `diagWrite(reason==='idle'?'idle-timeout':'hard-timeout', null)` after the give-up is decided.
    - APPENDS ONE JSON line to path.join(os.homedir(), 'jw-probe-timeline.jsonl') on SETTLE ONLY: { ts, sessionId, cwd, shell, idleBudgetMs(8000), hardBudgetMs(15000), firstByteMs, matchMs(null on timeout), outcome, maxSilentGapMs, byteCount, chunkCount, timeline:[{dtMs,size}…] }. SECRET-FREE — sizes + timings + non-secret lifecycle metadata (cwd/shell/sessionId) only; NEVER the nonce, marker, buffered bytes, or user cmd. appendFileSync wrapped in try/catch (a logging failure never perturbs lifecycle).
    - BEHAVIOR UNCHANGED (the absolute hard constraint): timers (idle 8000 reset-on-byte + hard 15000 absolute), the idle-extend reset, buffer forwarding, probe.matches, D-02 (no pre-match forward), D-04 (never inject on timeout), and the stale-timeout guard (`live.pty !== child`) are all untouched. The logging is observation only.
    - BUILD: `npx tsc --noEmit` exit 0 (type-clean). `npm run package` SUCCESS → /Users/jerry/Project/Just-wrapper/out/Just-Wrapper-darwin-arm64/Just-Wrapper.app (darwin arm64, built 17:02 2026-06-16, Mach-O executable present). DIAG CONFIRMED bundled inside Contents/Resources/app.asar (grep hit for both `jw-probe-timeline.jsonl` and `JW_PROBE_DIAG`; minified JSON shape matches source). Smoke suite intentionally NOT run (not needed for a diagnostic build).
  implication: The packaged app now writes the TRUE app-side probe timeline on every settle. This is the only path that captures the cold-Dock-launch first-spawn timeline the standalone instrument could not reproduce. HUMAN-ACTION checkpoint: the operator cold-launches the .app from Finder/Dock, reproduces GAP-12-B a few times, and pastes ~/jw-probe-timeline.jsonl. From the measured outcomes the fix disambiguates: idle-timeout with a silent gap >8000ms → reset-on-byte is the wrong liveness signal (larger idle window or a different signal); hard-timeout (total >15000ms) → raise the ceiling (and weigh a 'press Enter to run' affordance); match-in-app-but-still-fails → a different app-side spawn/lifecycle factor. NO budget number is changed until that data lands.
